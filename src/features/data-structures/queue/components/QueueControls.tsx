...
interface QueueControlsProps {
  hook: ReturnType<typeof useQueue>;
}

export default function QueueControls({ hook }: QueueControlsProps) {
  const {
  ...
  const handleEnqueue = () => {
    ...
    if (!isNaN(v)) { enqueue(v); setInputValue(''); }
  };
  ...
  return (
    <div className="flex flex-col gap-4 w-full md:w-72 shrink-0 overflow-y-auto max-h-full pr-1">
      ...
      <div className="rounded-xl border border-slate-700/50 bg-slate-900/60 p-4 backdrop-blur-sm">
        <h3 className="text-sm font-bold text-indigo-300 mb-1">Queue</h3>
        <p className="text-xs text-slate-400 leading-relaxed">
          A <strong className="text-slate-300">FIFO</strong> (First In, First Out) structure. Elements are added at the <strong className="text-slate-300">rear</strong> and removed from the <strong className="text-slate-300">front</strong>.
        </p>
        <div className="mt-2 flex flex-wrap gap-1.5">
          {['FIFO', 'Enqueue O(1)', 'Dequeue O(1)', 'Peek O(1)'].map((t) => (
          ...
          <div className="max-h-36 overflow-y-auto space-y-1">
            {history.map((h, i) => (
              <div key={i} className="flex items-center gap-2 text-xs">
                ...
                <span className={h.startsWith('Enqueue') ? 'text-emerald-400' : h.startsWith('Dequeue') ? 'text-rose-400' : h.startsWith('Peek') ? 'text-amber-400' : 'text-slate-400'}>
                  {h}
                </span>
              </div>
            ))}
      ...
      <div className="rounded-xl border border-slate-700/50 bg-slate-900/60 p-4 backdrop-blur-sm">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2"><div className="h-3 w-5 rounded-sm bg-emerald-500" /><span className="text-xs text-slate-400">Enqueue (rear)</span></div>
          <div className="flex items-center gap-2"><div className="h-3 w-5 rounded-sm bg-rose-500" /><span className="text-xs text-slate-400">Dequeue (front)</span></div>
