  ...
  return (
    <div className="flex flex-1 flex-col md:flex-row gap-4 md:gap-6 overflow-auto p-4 md:p-6">
      ...
      <div className="order-last md:order-first flex md:flex-1 flex-col gap-4 min-h-0 md:overflow-auto">
        {/* Both Players' Hands */}
        {players.map((p, pIdx) => (
          <div key={pIdx} className={`rounded-2xl border p-4 shadow-xl backdrop-blur-sm md:p-5 ${
            phase === 'building' && currentPlayer === pIdx
              ? pIdx === 0 ? 'border-indigo-500/30 bg-indigo-500/5' : 'border-violet-500/30 bg-violet-500/5'
              : 'border-emerald-900/40 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.12),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.96),rgba(6,78,59,0.28))]'
          }`}>
            ...
            <div className="flex min-h-[5rem] flex-wrap items-end gap-1.5 rounded-xl border border-emerald-950/40 bg-emerald-950/20 p-3 shadow-inner md:gap-2">
              {p.hand.map((code, i) => (
                <TpTile
                  key={`${code}-${i}`}
                  code={code}
              ...
              {p.hand.length < HAND_SIZE && Array.from({ length: HAND_SIZE - p.hand.length }).map((_, i) => (
                <MahjongTile key={`e-${i}`} size="md" placeholder />
              ))}
            </div>
          </div>
        ))}
        ...
        {phase === 'building' && !currentHandIsFull && (
          <div className="rounded-2xl border border-slate-700/50 bg-slate-900/70 p-4 shadow-xl shadow-black/20 backdrop-blur-sm md:p-5">
            <h3 className="text-xs md:text-sm font-semibold uppercase tracking-wider text-slate-300 mb-3">Tile Wall</h3>
            <div className="space-y-3 md:space-y-4">
              {SUITS.map((suit) => (
                <div key={suit} className="rounded-xl border border-slate-800/70 bg-slate-950/35 p-3">
                  ...
                  <div className="flex flex-wrap items-end gap-1 md:gap-1.5">
                    {TILE_VALUES.map((v) => {
