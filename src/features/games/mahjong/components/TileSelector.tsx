  ...
  return (
    <button
      ...
      className={`
        ...
        w-10 h-12 sm:w-12 sm:h-14 md:w-14 md:h-[4.25rem] lg:w-16 lg:h-[4.75rem] text-xs font-bold transition-all duration-150
        ${disabled || isFull
      ...
      `}
    >
      <span style={{ color: disabled || isFull ? undefined : color }} className="text-sm sm:text-base md:text-lg lg:text-xl font-extrabold leading-none">
        {value}
      </span>
      <span className="text-[8px] sm:text-[9px] md:text-[10px] text-slate-500 leading-none mt-0.5 md:mt-1">
        {suit === 'B' ? '竹' : suit === 'C' ? '万' : '●'}
      </span>
      {count > 0 && (
        <span className="absolute -top-1.5 -right-1.5 md:-top-2 md:-right-2 flex h-4 w-4 md:h-5 md:w-5 items-center justify-center rounded-full bg-slate-700 text-[9px] md:text-[10px] font-bold text-slate-300 ring-1 ring-slate-600">
          {count}
        </span>
      )}
    </button>
...
export default function TileSelector({ hand, handIsFull, onAddTile }: TileSelectorProps) {
  return (
    <div className="rounded-xl border border-slate-700/50 bg-slate-900/60 p-4 md:p-5 backdrop-blur-sm">
      <h3 className="text-xs md:text-sm font-semibold uppercase tracking-wider text-slate-400 mb-3 md:mb-4">Tile Palette</h3>
      <div className="space-y-4 md:space-y-5">
        {SUITS.map((suit) => (
          <div key={suit}>
            <p className="text-[10px] md:text-xs font-medium mb-1.5 md:mb-2" style={{ color: SUIT_COLORS[suit] }}>
              {SUIT_NAMES[suit]}
            </p>
