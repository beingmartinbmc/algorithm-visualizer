    ...
    <div className="flex flex-col gap-4 w-full md:w-72 shrink-0 overflow-y-auto max-h-full pr-1">
      ...
      <div className="rounded-xl border border-slate-700/50 bg-slate-900/60 p-4 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <button onClick={onRandomHand} className="flex items-center justify-center gap-1.5 rounded-lg bg-slate-800/60 px-3 py-2 text-xs font-medium text-slate-400 transition-all hover:bg-slate-700/60 hover:text-slate-300">
            <Shuffle size={12} /> Random
          </button>
          <button onClick={onWinningHand} className="flex items-center justify-center gap-1.5 rounded-lg bg-slate-800/60 px-3 py-2 text-xs font-medium text-slate-400 transition-all hover:bg-slate-700/60 hover:text-slate-300">
            <Sparkles size={12} /> Winning
          </button>
          <button onClick={onResetHand} className="flex items-center justify-center gap-1.5 rounded-lg bg-slate-800/60 px-3 py-2 text-xs font-medium text-slate-400 transition-all hover:bg-slate-700/60 hover:text-slate-300">
        ...
        >
          {showAnimation ? <Eye size={12} /> : <EyeOff size={12} />}
      ...
      {showAnimation && steps.length > 0 && (
        <div className="rounded-xl border border-slate-700/50 bg-slate-900/60 p-4 backdrop-blur-sm">
          <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3">Solver Steps</h3>
          <div className="flex gap-2 mb-3">
            <button onClick={onStepBack} disabled={stepIndex <= -1} className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-slate-800/60 px-2 py-2 text-xs font-medium text-slate-400 hover:bg-slate-700/60 hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed">
              <ChevronLeft size={14} /> Prev
            </button>
            <button onClick={onAutoPlay} className={`flex-1 flex items-center justify-center gap-1 rounded-lg px-2 py-2 text-xs font-medium transition-all ${isAnimating ? 'bg-amber-500/20 text-amber-300 ring-1 ring-amber-500/30' : 'bg-indigo-500/20 text-indigo-300 ring-1 ring-indigo-500/30 hover:bg-indigo-500/30'}`}>
              {isAnimating ? <><Pause size={14} /> Pause</> : <><Play size={14} /> Play</>}
            </button>
            <button onClick={onStepForward} disabled={stepIndex >= stepsTotal - 1} className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-slate-800/60 px-2 py-2 text-xs font-medium text-slate-400 hover:bg-slate-700/60 hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed">
              Next <ChevronRight size={14} />
            </button>
          </div>
          <div className="flex gap-1 mb-2">
            {speeds.map((s) => (
              <button key={s.value} onClick={() => onSetAnimSpeed(s.value)} className={`flex-1 rounded-md px-2 py-1 text-[10px] font-medium transition-all ${animSpeed === s.value ? 'bg-indigo-500/20 text-indigo-300 ring-1 ring-indigo-500/30' : 'bg-slate-800/50 text-slate-500 hover:text-slate-400'}`}>
                {s.label}
              </button>
            ))}
          </div>
      ...
      {result && !showAnimation && (
        <div className={`rounded-xl border p-4 backdrop-blur-sm ${
          ...
          <div className="flex items-center gap-2">
            <Trophy size={14} className={result.isWin ? 'text-emerald-400' : 'text-red-400'} />
            <span className={`text-sm font-bold ${result.isWin ? 'text-emerald-300' : 'text-red-300'}`}>
